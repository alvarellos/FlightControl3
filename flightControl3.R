...
df2 <- read.csv("C://Users/Diego/Documents/R/2007.csv")
df3 <- read.csv("C://Users/Diego/Documents/R/2008(1).csv")
...
# Table with unic origin and destination
...
airportsDF <- read.csv("C://Users/Diego/Documents/R/airports.csv")
dim(airportsDF)
...
# the function "activeairports" that we designed on each element of the built-in vector state.abb 
# and compare your results with the number of airports in Indiana, Illinois, and California
# We included the "[1]" there so that we could only see the first dimension of the result.
...
MySubDataset <- subset(myDF, myDF$Origin == "LAX")
dim(MySubDataset)

# Quiz 18
# Question 1
# How many flights were there from IND to ORD from 2006 to 2008?
MySubDataset <- subset(myDF, myDF$Origin == "IND" & myDF$Dest == "ORD")
dim(MySubDataset)

# Question 2
# Make a table that counts the number of entries (simultaneously) 
# according to the origin, destination, and year. How many flights 
# were there from BOS to DEN in 2007? Use the same data frame we already built. 
# Do not reload all of the 2007 data.

#Using the dataframe
sum(myDF$Year == "2007" & myDF$Origin == "BOS" & myDF$Dest == "DEN")

# Creating the table
table(paste(myDF$Year, myDF$Origin, myDF$Dest)) 
Flightpaths3 <- table(paste(myDF$Year, myDF$Origin, myDF$Dest)) 
Flightpaths3["2007 BOS DEN"]

# Quiz 19
# According to the data in the airports.csv file, how many airports are located in the City of Chicago?

# List
subset(airportsDF, city == "Chicago")

# Number
sum(airportsDF$city=="Chicago", na.rm = T)


# Airports that contain the word chicago
# subset(airportsDF, state == "IL" & startsWith(as.character(city), "Chicago"))

# Quiz 20
# Combining the information from the airports.csv file and from the 2006 to 2008 airline data sets, 
# how many airports in California had at least one flight in the data set during this time period (2006-2008)?
subset(airportsDF, state=="CA") 
caliairp<- subset(airportsDF, state=="CA") 
caliairp$iata 
as.character(caliairp$iata) 
table(myDF$Origin)[as.character(caliairp$iata)] 
e<- table(myDF$Origin)[as.character(caliairp$iata)] 
e[!is.na(e)] 
names(e[!is.na(e)]) 
subset(airportsDF, iata%in% names(e[!is.na(e)])) 
dim(subset(airportsDF, iata%in% names(e[!is.na(e)])))
...
datestring <- paste(myDF$DayofMonth, myDF$Month, myDF$Year, sep="/") 
head(datestring) 
date <- as.Date(datestring, "%d/%m/%Y") 
head(date) 
tail(sort(tapply(myDF$TailNum, date, length)))
